...
int mcp2210_subcommand(int fd, mcp2210_packet packet, unsigned short command,
                       unsigned short subcommand);
int mcp2210_read_eeprom(int fd, mcp2210_packet packet, unsigned short addr);
int mcp2210_write_eeprom(int fd, mcp2210_packet packet, unsigned short addr,
                         unsigned short val);
...
static inline int mcp2210_get_command(int fd, mcp2210_packet packet,
                                      unsigned short command) {
  memset(packet, 0, MCP2210_PACKET_SIZE);
  return mcp2210_command(fd, packet, command);
}

static inline int mcp2210_get_nvram(int fd, mcp2210_packet packet,
                                    unsigned short subcommand) {
  memset(packet, 0, MCP2210_PACKET_SIZE);
  return mcp2210_subcommand(fd, packet, MCP2210_NVRAM_GET, subcommand);
}

static inline int mcp2210_set_nvram(int fd, mcp2210_packet packet,
                                    unsigned short subcommand) {
...
static inline void mcp2210_chip_set_function(mcp2210_packet chip_packet,
                                             unsigned short pin,
                                             unsigned short func) {
...
static inline unsigned short
mcp2210_chip_get_default_output(mcp2210_packet chip_packet,
                                unsigned short pin) {
  return ((chip_packet[14] << 8 | chip_packet[13]) >> pin) & 1;
}

static inline void mcp2210_chip_set_default_output(mcp2210_packet chip_packet,
                                                   unsigned short pin,
                                                   unsigned short val) {
  if (pin < 8) {
    if (val) {
...
static inline unsigned short
mcp2210_chip_get_default_direction(mcp2210_packet chip_packet,
                                   unsigned short pin) {
  return ((chip_packet[16] << 8 | chip_packet[15]) >> pin) & 1;
}

static inline void
mcp2210_chip_set_default_direction(mcp2210_packet chip_packet,
                                   unsigned short pin, unsigned short val) {
  if (pin < 8) {
    if (val) {
...
static inline void mcp2210_chip_set_wakeup(mcp2210_packet chip_packet,
...
static inline void mcp2210_chip_set_gp6_mode(mcp2210_packet chip_packet,
                                             unsigned short mode) {
  chip_packet[17] &= ~0xe;
...
static inline void mcp2210_chip_set_no_spi_release(mcp2210_packet chip_packet,
                                                   unsigned short no_release) {
  chip_packet[17] &= ~0x1;
...
static inline void mcp2210_chip_set_access_control(mcp2210_packet chip_packet,
                                                   unsigned short setting) {
  chip_packet[18] = setting;
}

static inline void mcp2210_chip_set_access_password(mcp2210_packet chip_packet,
                                                    const char *passwd) {
 ...
 * value and pin directions. Use with MCP2210_GPIO_VAL_SET,
 * MCP2210_GPIO_VAL_GET, MCP2210_GPIO_DIR_SET and MCP2210_GPIO_DIR_GET
 * respectively.
 */

static inline unsigned short mcp2210_gpio_get_pin(mcp2210_packet gpio_packet,
                                                  unsigned short pin) {
  return ((gpio_packet[5] << 8 | gpio_packet[4]) >> pin) & 1;
}

static inline void mcp2210_gpio_set_pin(mcp2210_packet gpio_packet,
                                        unsigned short pin,
                                        unsigned short val) {
  if (pin < 8) {
    if (val) {
...
static inline long mcp2210_spi_get_bitrate(mcp2210_packet spi_packet) {
  return spi_packet[4] | (spi_packet[5] << 8) | (spi_packet[6] << 16) |
         (spi_packet[7] << 24);
}

static inline void mcp2210_spi_set_bitrate(mcp2210_packet spi_packet,
                                           long bitrate) {
...
static inline void mcp2210_spi_set_pin_active_cs(mcp2210_packet spi_packet,
                                                 unsigned short pin,
                                                 unsigned short val) {
  if (pin < 8) {
    if (val) {
...
static inline void mcp2210_spi_set_pin_idle_cs(mcp2210_packet spi_packet,
                                               unsigned short pin,
                                               unsigned short val) {
  if (pin < 8) {
    if (val) {
...
static inline void
mcp2210_spi_set_cs_data_delay_100us(mcp2210_packet spi_packet,
                                    unsigned int delay_100us) {
...
static inline void
mcp2210_spi_set_data_cs_delay_100us(mcp2210_packet spi_packet,
                                    unsigned int delay_100us) {
...
static inline void mcp2210_spi_set_byte_delay_100us(mcp2210_packet spi_packet,
                                                    unsigned int delay_100us) {
...
static inline void mcp2210_spi_set_transaction_size(mcp2210_packet spi_packet,
                                                    unsigned int size) {
  spi_packet[18] = size & 0xff;
  spi_packet[19] = (size >> 8) & 0xff;
}

static inline unsigned short mcp2210_spi_get_mode(mcp2210_packet spi_packet) {
  return spi_packet[20];
}

static inline void mcp2210_spi_set_mode(mcp2210_packet spi_packet,
                                        unsigned short mode) {
  spi_packet[20] = mode;
}
...
static inline void mcp2210_usb_key_set_vid(mcp2210_packet usb_key_packet,
                                           unsigned short vid) {
...
static inline void mcp2210_usb_key_set_pid(mcp2210_packet usb_key_packet,
                                           unsigned short pid) {
  if (usb_key_packet[0] == MCP2210_NVRAM_GET) {
...
static inline void
mcp2210_usb_key_set_host_powered(mcp2210_packet usb_key_packet,
                                 unsigned short on) {
  if (usb_key_packet[0] == MCP2210_NVRAM_GET) {
...
static inline void
mcp2210_usb_key_set_self_powered(mcp2210_packet usb_key_packet,
                                 unsigned short on) {
  if (usb_key_packet[0] == MCP2210_NVRAM_GET) {
...
static inline void
mcp2210_usb_key_set_remote_wakeup(mcp2210_packet usb_key_packet,
                                  unsigned short on) {
  if (usb_key_packet[0] == MCP2210_NVRAM_GET) {
...
static inline void
mcp2210_usb_key_set_current_2ma(mcp2210_packet usb_key_packet,
                                unsigned short current) {
  usb_key_packet[usb_key_packet[0] == MCP2210_NVRAM_GET ? 30 : 9] = current;
}

static inline void mcp2210_usb_key_get_to_set(mcp2210_packet get,
                                              mcp2210_packet set) {
...
static inline char *mcp2210_usb_string_get(mcp2210_packet packet) {
  return (char *)&packet[6];
}

static inline unsigned short mcp2210_usb_string_get_len(mcp2210_packet packet) {
  return packet[4] - 2;
}

static inline void mcp2210_usb_string_set(mcp2210_packet packet,
                                          const char *string, short len) {
